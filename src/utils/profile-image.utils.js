...
const BADGES_DIR = path.join(__dirname, '..', '..', '22', 'assets');
let badgeMap = null;
...
function loadBadgeMap() {
  ...
  if (!fs.existsSync(BADGES_DIR)) return badgeMap;
  ...
  try {
    ...
    console.warn('Error loading local badges:', error.message);
  }
  return badgeMap;
}
...
async function getBadges(data, options) {
  ...
  const canvasBadges = [];
  const localMap = loadBadgeMap();

  for (const badge of badges.reverse()) {
    const { name, icon } = badge;
    
    const normalized = name.toLowerCase().replace(/[^a-z0-9]/g, '');
    let localPath = localMap[normalized];
    if (!localPath && localMap['discord' + normalized]) {
      localPath = localMap['discord' + normalized];
    }
    
    let canvas;
    if (localPath) {
      canvas = await loadImage(localPath).catch(() => {});
    }

    if (!canvas) {
      canvas = await loadImage(icon).catch(() => {});
    }
